...
class AMP_ActiChamp(ModuleBase):
    ...
    def process_start(self):
        ...
        bytesnum = int((4+cnum*pnum)*8)

        self.shm.point = pnum

        self.shm.sharemem = mmap.mmap(0,bytesnum,access=mmap.ACCESS_WRITE,tagname='__eeg_from_pycorder__')
        head = np.array([samplerate,cnum,pnum],dtype=np.float64).tostring()
